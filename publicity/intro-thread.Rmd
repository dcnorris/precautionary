...
```{r tweet=c("My initial experiments toward #precautionary began by cloning the sourcecode for #escalation, and --- following the plan I had intimated in a 2018 letter desk-rejected by @CCR_AACR --- \"changing just one line of simulation code\": https://pubpeer.com/publications/4CEB4434EA94CC0AA3BCAF63617A78#1")}
knitr::include_graphics("an-extremely-modest-proposal.png")
```


```{r tweet=c("Here's the git diff (red=removed; green=added). Admittedly, I split the change across 2 lines of code:","• cohorts_of_n() now generates a uniform random variate $u_i \\sim \\mathscr{U}[0,1]$ for each enrolling patient","• phase1_sim() then refers to $u_i$ instead of calling rbinom().")}
...
```{r tweet=c("Wait, though ... If someone destroys your universe, but you're not paying attention, does your universe endure? https://twitter.com/davidcnorrismd/status/1244471654882750466")}
...
```{r tweet=c("The CTCAE toxicity grading system implicitly acknowledges this, I think. The numerator / denominator of an oncology-adapted TI ought to reference doses at which unacceptable / tolerable toxicities occur."), link="https://twitter.com/davidcnorrismd/status/1284451892224053250"}
```
...
```{r tweet=c("For line-by-line details of R commands, I refer you to the #precautionary package's Intro vignette [link here]. But the basic outline is like this:","We start from where the #escalation package leaves off:")}
...
```{r tweet=c("Only 2 weeks ago, @US_FDA put a clinical hold on a fatal CAR-T dose escalation.","And the abovementioned arXiv paper, where MTD$_i^g$ and $r_0$ arose, was stimulated by severe & fatal toxicities in #AFM11 dose-escalation trials."), link=c("https://twitter.com/statnews/status/1280501925859950600","https://twitter.com/davidcnorrismd/status/1255120071648821249")}
```
...
```{r tweet=c("This is an INITIAL RELEASE of #precautionary --- v0.1-1. Much further work remains: to test & extend the package, and to automate certain functionality. I hope to carry out most of this effort in collaborations on investigator-initiated trials. Investigators: email or DM me!")}
```


```{r tweet=c("I also must explore enlarging the #precautionary tent to subsume other R #dosefinding packages, such as #crmPack by @thomas_jaki &al. Might suitable defaults exist to enable semi-automated safety checks on any off-the-shelf dose-escalation design?"), link="https://twitter.com/thomas_jaki/status/1229462881898565632"}
```
