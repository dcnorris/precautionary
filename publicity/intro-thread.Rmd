...
output:
  html_document:
    keep_md: true
#    pandoc_args: ["--lua-filter=recover-asterisks.lua"]
---
...
```{r tweet=c("New #rstats package #precautionary [pending] on CRAN","\"The shaft of the arrow had been feathered with one of the eagle's own plumes. We often give our enemies the means of our own destruction.\"\n\n --- Aesop\n\n[THREAD]"), link="https://CRAN.R-project.org/package=precautionary"}
```
...
```{r tweet=c("My initial experiments toward #precautionary began by cloning the source for #escalation, and --- following the plan I had intimated in a 2018 letter desk-rejected by @CCR_AACR --- \"changing just one line of simulation code\": https://pubpeer.com/publications/4CEB4434EA94CC0AA3BCAF63617A78#1")}
knitr::include_graphics("an-extremely-modest-proposal.png")
```


```{r tweet=c("Here's the git diff (red=removed; green=added). Admittedly, I split the change across 2 lines of code:","• cohorts_of_n() now generates a uniform random variate $u_i \\sim \\mathscr{U}(0,1]$ for each enrolled patient","• phase1_sim() then uses $u_i$ instead of calling rbinom().")}
...
```{r tweet=c("Seen from a purely *software-engineering* POV, these are simple code transformations: we 'unwrapped' rbinom() to expose its inner workings, retaining u_i.","But this formal refactoring turns out to have *conceptual* correlates. Retaining $u_i$ makes a world of difference!")}
```
...
```{r tweet=c("What exactly is the litany of ills we have unleashed by opening up rbinom()? You know how on #DoctorWho some villain is always going to destroy the fabric of the universe?","Well, that's basically what u_i does. (Only, it's not ME that's evil, it's THEM!)"), link="https://twitter.com/TygerWhoCame2T/status/1251955542525968386"}
```

---

```{r tweet=c("Wait, though ... If someone destroys your universe, but you're not paying attention, does your universe remain intact? https://twitter.com/davidcnorrismd/status/1244471654882750466")}
...
```{r tweet=c("...or, more precisely, a $u_i$ for each patent in the simulation.","With these $u_i$'s, we can reconstitute most of the #DoseIndividualization argument right under the #OneSizeFitsAllogists' noses, in the midst of one of their dose-escalation trials. We need only 1 further idea...")}
```
...
```{r tweet=c("In their short section on Oncology indications, Muller & Milton address the substantial toxicity of many oncology drugs in terms of a 'TI < 1'. But this seems to carry over inappropriate TI notions from other clinical applications.")}
knitr::include_graphics("onc-TI<1.png")
```


```{r tweet=c("If a #TherapeuticIndex ratio is to remain useful in a clinical context like oncology, where toxicity is expected and sometimes sought-after as a clinical correlate of on-target efficacy, then a less stringent numerator is needed."), link="https://twitter.com/davidcnorrismd/status/1176114631951159296"}
```


```{r tweet=c("The CTCAE toxicity grading system implicitly acknowledges this, I think. The numerator and denominator of an oncology-adapted TI ought to reference doses at which unacceptable and tolerable toxicities occur."), link="https://twitter.com/davidcnorrismd/status/1284451892224053250"}
```
...
```{r tweet=c("And now an $i$ subscript on TI, too? Well, ... YES!", "In person-centered care you have every reason to consider TI as a patient-specific concept. When you advise, \"I don't think this would be a good drug for you\", you're saying the patient's TI$_i$ is not sufficiently above 1.")}
```


```{r tweet=c("This TI$_i$ concept now looks like the $r_i$ of that arXiv paper, a dose ratio that connects adjacent CTCAE toxicity grades.", link="https://twitter.com/davidcnorrismd/status/1255160313340723203")}
```
...
```{r tweet=c("For present purposes, tho, let's drop the $i$ subscript, since a population-average TI notion suffices. Thus, as in the arXiv paper, we write $r_0$ instead of $r_i$.","Now here's the deal: You posit an $r_0$; #precautionary simulates your trial's safety."), link="https://twitter.com/davidcnorrismd/status/1255165059807969287"}
```


```{r tweet=c("For line-by-line details of R commands, I refer you to the #precautionary package's Intro vignette. But the basic outline is like this:","We start from where the #escalation package leaves off:")}
...
```{r tweet=c("Secondly, because #escalation works at a high level of abstraction, such that doses are merely labeled '1', '2', '3', ..., we have to specify the actual doses in our trial.")}
...
```{r tweet=c("When we invoke the simulate_trials() method now, #precautionary takes over---through object-oriented 'magic'.","So far, little has changed about the output. But behind the scenes, SIMS now contains the u_i variable that #escalation's simulation output lacked.")}
...
```{r tweet=c("But why do I offer this up as some kind of catastrophe for the #OneSizeFitsAllogists?","Because once people start thinking this way, they won't be able to stop. Three decades of evading optimal-dose heterogeneity will come to screeching halt.")}
```


```{r tweet=c("Fatalities in phase 1 oncology trials do happen, and they reflect poorly on 1-size-fits-all #dosefinding designs. Only 2 weeks ago, @US_FDA placed a clinical hold on a CAR-T dose-escalation trial:"), link="https://twitter.com/statnews/status/1280501925859950600"}
```


```{r tweet=c("Indeed the abovementioned arXiv paper, where the MTD$_i^g$ and $r_0$ concepts were developed, was stimulated by severe and fatal toxicities in two #AFM11 trials."), link="https://twitter.com/davidcnorrismd/status/1255120071648821249"}
```


```{r tweet=c("Surely @FDAOncology #prospectively reviews phase 1 trial designs under 21 CFR §312.42(b)(1)(i). Such reviews will be more effective, and make trials #safer, if they take into view the #TherapeuticIndex considerations implemented in #precautionary."), link="https://twitter.com/davidcnorrismd/status/1264186764161363975"}
```


```{r tweet=c("In the final analysis, it is of course the physician-investigator who bears the ultimate responsibility for trial safety. My greatest hope is that package #precautionary helps trialists to think more realistically about #dosefinding designs.")}
```


```{r tweet=c("This is only version 0.1-1 of #precautionary! Much further work remains to test and extend the package, especially to automate some functionality. I hope some of this work can be done in collaborations on investigator-initiated trials. Email or DM!")}
```


```{r tweet=c("There may also be some value in extending the reach of #precautionary to other R packages for #dosefinding trial design. The application of reasonable defaults might enable semi-automated safety analysis for off-the-shelf dose-escalation designs."), link="https://twitter.com/thomas_jaki/status/1229462881898565632"}
```
