...
The DTP apparently was introduced primarily as a means to reconcile model-based formulations of dose-escalation trials with the habitually rule-based thinking of clinicians. Specifically, @yap_dose_2017 cites 3 specific "perceived challenges" in making the transition from rule-based to model-based designs:
...
To meet these challenges, DTP effectively transforms a model-based design into a rule-based design *locally* --- so that its immediate operation several steps ahead can be 'eyeballed' by trialists as a small set of (e.g., $4^4 = 64$) distinct paths. The exhaustive enumeration employed in @norris_what_2020 (and here), differs only in its *global* reach and its intent to support computation over the paths as opposed to 'eyeballing'.
...
The development of CRM methods has left the status of the *skeleton* somewhat ambiguous, largely evading its realistic interpretation as an $\\MTDi$ distribution.^[For a related perspective on interpretational issues, see @norris_comment_2020.] Nevertheless, CRM skeletons have served as natural starting points for simulation studies. @braun_simulation-free_2020, for example, develops an approach which "hinges on the assumpion that the true DLT probabilities examined are consistent with the selected skeleton."
...
To enable parameter departures from our lognormal skeleton fit, we require $\log(\pi)$ as a *function* of $(\mu, \sigma)$:
...
This enables us to compute expected fatalities as a scalar field on a 3-dimensional space defined by parameters $(\mu, \sigma, \kappa)$. But as in @norris_what_2020 we employ a minimax framing of our safety question, to achieve a dimension reduction:

```{r minimax}
mu_minimax <- log(viola_dosing$Dose_mg)[3] # dose 3 is 2nd-highest *nonzero* dose
```

The irregularly spaced doses of VIOLA don't yield a unique delta, but the 2x mulipliers either side of dose level 3 (our mu_minimax) allow us to select delta = log(2) as a local approximation:

```{r delta}
delta <- mean(diff(log(viola_dosing$Dose_mg)[2:4]))
```

If I am to obtain expected fatalities as a scalar field over the (kappa/sigma)--(delta/sigma) plane, then given that delta is fixed, we obtain our range of sigmas thus:

```{r kappa-delta-plane}
focustab <- CJ(mu = mu_minimax
              , K = seq(0.5, 1.65 , 0.01) # K = kappa / sigma
              , sigma = delta/seq(0.4, 2.2, 0.01) # so 0.5 < delta/sigma < 2.0
              )
focustab[, kappa := K * sigma]
focustab[, value := t(exp(log_pi(mu,sigma))) %*% Y %*% f(kappa=kappa, mu=mu, sigma=sigma)
       , by = .(mu,sigma)]
```
...
# Potential for irrational dose assignment

TODO: Analyze the `viola_paths` matrix for occurrences of 'irrational dose assignment'. Obviously, ths is a point where a Prolog implementation would shine!

# Conclusion

Although initially put forward with different intent, the *dose transition pathways* of @yap_dose_2017 directly correspond to the enumerative approach employed in @norris_what_2020 to investigate safety properties of the 3+3 design, enabling a cognate analysis for the CRM. Importantly, the analysis is seen to be eminently feasible in the setting of an actual trial. Moreover, the only nontrivial computation time involved in this approach manifests an 'embarrassing' degree of parallelism that might enable a 10$\times$ speedup even on a single modern CPU or even effectively zero out the computation time in a massively parallel GPU implementation.

More broadly, the approach taken here illustrates a general scheme for safety analysis in dose-escalation trials generally, starting from the exhaustive enumeration of all possible trial outcomes.
