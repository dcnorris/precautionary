...
Paths terminating at the 5th cohort would be listed 16 times, and those terminating at the 4th cohort 64 times, etc. The net degeneracy indeed proves to be quite substantial:
```{r degeneracy}
viola_paths <- as.matrix(unique(viola_dtp))
nrow(viola_paths)
```

# Computing $\mathbf{b}$ and $\mathrm{U}$ from the DTP table

The vector $\mathbf{b} = \sum_c {n \choose T_{c,d}}$ may be obtained directly from the DTP table, without actually constructing the path matrices $T^j$:

```{r b-directly}
b <- apply(log(choose(3, viola_paths[, paste0("T",1:7)])),
           MARGIN = 1, FUN = sum)
length(b)
```

The matrix $\mathrm{Y}_d^j = \sum_c T_{c,d}^j$, however, does seem most straightforward to calculate from the full 3-dimensional array $T_{c,d}^j$:

```{r T-constructed}
I <- outer(viola_paths[,paste0("D",0:6)], 1:7, FUN = "==")
# Now I[j,c,d] is an indicator array that we may use
# to select the toxicities into the correct positions
# within T[j,c,d]:
...
With this accomplished, $\mathrm{Y}_d^j$ is easily obtained:

```{r Y-soeasy}
Y <- apply(T, MARGIN = c(1,3), FUN = sum)
dim(Y)
```
