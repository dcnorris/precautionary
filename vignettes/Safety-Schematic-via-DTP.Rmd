...
q_d &= 1 - p_d\\
\pi^j &= \prod_{c,d} {3 \choose T_{c,d}^j} p_d^{T_{c,d}^j} q_d^{(3-T_{c,d}^j)}\quad\mbox{(path probabilities)} (\#eq:pi)
\end{align}
$$
...
$$
\log \boldsymbol{\pi} = \sum_{c,d} \log {3 \choose T_{c,d}} + \sum_c [T_{c,d},3-T_{c,d}]\left[{ \log \mathbf{p} \atop \log \mathbf{q}} \right] = \mathbf{b} + U\left[{ \log \mathbf{p} \atop \log \mathbf{q}} \right], (\#eq:logpi)
$$
...
TODO: Calculate the T array for the 'non-negative' part of the Viola trial, which had 5 iregularly spaced LEN doses: 5, 10, 15, 25 and 35 mg. Note that there is an opportunity here to meet the challenge of accommodating non-logarithmic dose scaling as might be encountered in practice.
