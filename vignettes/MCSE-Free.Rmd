...
A total of `r dim(T)[1]` paths have been enumerated, for which toxicities are tabulated separately for as many as `r dim(T)[2]` distinct cohorts per each of `r dim(T)[3]` doses.
...
## Derive $\mathbf{\pi}$ from the CRM skeleton, checking that $\sum_j \pi^j \equiv 1$ 

```{r pi-per-skeleton}
log_p <- environment(calmod$initialize)$private$ln_skel # TODO: Provide an accessor!
log_q <- log(1 - exp(log_p))
