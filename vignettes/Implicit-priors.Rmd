...
U <- precautionary:::U
D <- 4
# Build a 4-dimensional array (r0) x (rho) x (MDT50) x (SDLOG)
...
rho <- r0 <- exp(seq(log(1.3), log(2.0), length.out = 8))
MTD_median <- seq(1, D, 0.25) # express dose on scale of dose number 1--D (fractions ok!)
MTD_SDlog <- seq(0.3, 1.2, 0.1) # TODO: Better scaled relative to rho, r0, or both?
F <- array(dim = sapply(list(r0, rho, MTD_median, MTD_SDlog), length),
           dimnames = list(r0=r0,
                           rho=rho,
                           MTD_median=MTD_median,
                           MTD_SDlog=MTD_SDlog))
for (q in 1:length(rho)) {
  doses <- rho[q]^(seq(D) - (D+1)/2)
  stopifnot(all.equal(0, mean(log(doses)))) # geometric mean of dose range is 1
  for (s in 1:length(MTD_SDlog)) {
    for (t in 1:length(MTD_median)) {
      meanlog <- log(doses[1]) + log(doses[D]/doses[1])*(MTD_median[t]-1)/(D - 1)
      sdlog <- MTD_SDlog[s]
      ## ** Here is where the various scales interact, and
      ##    where dimensionless constants are to be found! **
...
fataltab[, `:=`(
  r0 = ordered(paste("r0=", round(as.numeric(r0),2)))
, rho = ordered(paste("rho=", round(as.numeric(rho),2)))
, MTD_median = as.numeric(MTD_median)
, MTD_SDlog = as.numeric(MTD_SDlog)
)]
contourplot(value ~ MTD_median + MTD_SDlog | r0 * rho
            , data = fataltab
            , cuts = 10)
```
