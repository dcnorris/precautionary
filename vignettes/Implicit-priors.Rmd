...
```{r compare-sim}
library(precautionary)
# Note we override the default to use what Korn &al (1994) Stat Med
# called the 'more common' variant of the "standard method". This also
# aligns with the 3+3 method as defined in Skolnik &al (2008) JCO.
...
```{r super-sim}
SIMS %>% extend(target_mcse = 0.05) -> SUPERSIMS
...
j <- function(m) {
  D <- ncol(m)
  J <- dim(A[[D]])[3]
  which(sapply(1:J, function(j) identical(m, A[[D]][,,j])))
}
j(m(fit))
```
