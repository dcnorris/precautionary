...
D <- 4
rho <- 1.5
doses <- rho^(seq(D) - (D+1)/2)
stopifnot(all.equal(0, mean(log(doses)))) # geometric mean of dose range is 1
###options(dose_levels = doses)
###design <- get_three_plus_three(num_doses = D, allow_deescalate = TRUE)
# TODO: Examine these heuristics more closely
median_MTDs <- seq(min(doses), max(doses), length.out = 11)
median_SDlogs <- seq(0.3, 1.2, 0.1)
b <- precautionary:::b
U <- precautionary:::U
T <- length(median_MTDs)
S <- length(median_SDlogs)
R <- length(
  r0_scenarios <- seq(1.3, 3.0, 0.1))
fatal_detail <- array(dim = c(length(r0_scenarios),
                              length(median_MTDs),
                              length(median_SDlogs)),
                      dimnames = list(r0=r0_scenarios,
                                      medianMTD=median_MTDs,
                                      medianSDlog=median_SDlogs)
                      )
for (s in 1:S) {
  for (t in 1:T) {
    p <- plnorm(doses, meanlog = log(median_MTDs[t]), sdlog = median_SDlogs[s])
    ...
    Ftox <- plnorm((doses %o% r0_scenarios^-2), # CDF pullback to fatal thresholds
                   meanlog = log(median_MTDs[t]),
                   sdlog = median_SDlogs[s])
    # Ftox is a (D x R) matrix of probabilities of fatal toxicity at the D doses
    # under each of R r0_scenarios.
    fatal_detail[,t,s] <- piTxU %*% Ftox # D row-vector x (D*R) matrix
  }
}
par(mar = c(5,5,2,2)) # make room for horizontal left-axis title
contour(x = median_SDlogs, y = r0_scenarios/rho
        , z=t(apply(fatal_detail, MARGIN=c(1,3), FUN=mean))
