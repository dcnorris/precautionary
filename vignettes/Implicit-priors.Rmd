...
U <- precautionary:::U
S <- length(
  CV_scenarios <- seq(0.3, 1.2, 0.05))
R <- length(
  r0_scenarios <- seq(1.3, 2.0, 0.05))
...
contour(x = CV_scenarios, y = r0_scenarios, z=t(fatal_topo)
        , xlab = "CV", ylab = " " #expression(r[0])
        , las = 1)
mtext(expression(r[0]), side = 2, line = 3, las = 1)
```
