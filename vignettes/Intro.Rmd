...
One reason that package `escalation` makes such a suitable basis for these developments is that it unifies multiple dose-escalation designs. Thus, we can repeat the above simulation exercise with a CRM design:
...
crm_design <- get_dfcrm(skeleton = scenario, target = 0.25) %>%
  stop_at_n(n = 24)
crm_design %>% simulate_trials(
  num_sims = 20 # interpreted same as c(20,20), for convenience
